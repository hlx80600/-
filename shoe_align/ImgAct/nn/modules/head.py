...
from ImgAct.utils import NOT_MACOS14
from ImgAct.utils.tal import dist2bbox, dist2rbox, make_anchors
from ImgAct.utils.torch_utils import TORCH_1_11, fuse_conv_and_bn, smart_inference_mode
...
__all__ = (
    ...
    "Classify",
    "Detect",
    "DiscreteMultiActionHead",
    "MultiActionHead",
    "Pose",
    "RTDETRDecoder",
    "Segment",
    "SemanticSegment",
    "TargetMultiActionDetect",
    "YOLOEDetect",
    "YOLOESegment",
    "v10Detect",
)
...
class MultiActionHead(nn.Module):
    """YOLO multi-head classification head for predicting multiple independent labels simultaneously.

    Each head outputs a probability distribution over its own set of classes. For example, predicting
    movement direction in X, Y, Z axes where each axis has classes {-1, 0, 1}.

    Attributes:
        export (bool): Export mode flag.
        num_heads (int): Number of classification heads.
        classes_per_head (int): Number of classes per head.
        conv (Conv): Shared convolutional layer for feature transformation.
        pool (nn.AdaptiveAvgPool2d): Global average pooling layer.
        drop (nn.Dropout): Dropout layer for regularization.
        linears (nn.ModuleList): List of linear layers, one per head.
        head_names (list[str]): Names for each head (e.g., ['X', 'Y', 'Z']).
        class_names (list[str]): Names for classes within each head (e.g., ['-1', '0', '1']).
    """

    export = False  # export mode

    def __new__(cls, c1: int | str | dict | None = None, c2: int | None = None, *args, **kwargs):
        """Construct a bare head from channels or a full classification model from a YAML config."""
        if isinstance(c1, (str, dict)):
            from ImgAct.nn.tasks import ClassificationModel

            ch = kwargs.pop("ch", c2 if c2 is not None else 3)
            nc = kwargs.pop("nc", args[0] if args else None)
            verbose = kwargs.pop("verbose", args[1] if len(args) > 1 else True)
            if len(args) > 2:
                raise TypeError("YAML construction accepts at most cfg, ch, nc, verbose.")
            if kwargs:
                unexpected = ", ".join(sorted(kwargs))
                raise TypeError(f"Unexpected keyword arguments for YAML construction: {unexpected}")
            return ClassificationModel(c1, ch=ch, nc=nc, verbose=verbose)
        return super().__new__(cls)

    def __init__(self, c1: int | str | dict, c2: int | None = None, num_heads: int = 3, classes_per_head: int = 3,
                 k: int = 1, s: int = 1, p: int | None = None, g: int = 1):
        """Initialize multi-head classification head.

        Args:
            c1 (int): Number of input channels.
            c2 (int): Unused (kept for compatibility), actual output is num_heads * classes_per_head.
        ...
        """
        if c2 is None:
            raise TypeError("c2 must be provided when constructing MultiActionHead directly.")
        super().__init__()
        c_ = 1280  # efficientnet_b0 size
        ...
        self.drop = nn.Dropout(p=0.0, inplace=True)
        self.linears = nn.ModuleList([nn.Linear(c_, classes_per_head) for _ in range(num_heads)])

    def forward(self, x: list[torch.Tensor] | torch.Tensor) -> torch.Tensor | tuple:
        """Forward pass: returns (batch, num_heads * classes_per_head) continuous regression values."""
        if isinstance(x, list):
            x = torch.cat(x, 1)
        feat = self.drop(self.pool(self.conv(x)).flatten(1))  # (batch, c_)
        logits = torch.cat([linear(feat) for linear in self.linears], dim=1)  # (batch, num_heads * classes_per_head)
        return logits


class DiscreteMultiActionHead(MultiActionHead):
    """Discrete classification version of MultiActionHead.
    
    Identical network structure, but explicitly named so the task initialization
    knows to use DiscreteMultiActionHeadLoss (CrossEntropy) instead of SmoothL1.
    """
    pass


class TargetMultiActionDetect(Detect):
    """YOLO detection head with per-object multi-action direction prediction.

    Extends Detect to predict movement direction (X, Y, Z) for each detected object.
    Each detection gets additional action predictions: num_actions heads, each with classes_per_action classes.

    For example, with action_shape=(3, 3):
        - 3 action heads (X, Y, Z)
        - 3 classes per head (-1, 0, 1)
        - Total 9 extra values per detection

    Attributes:
        action_shape (tuple): (num_actions, classes_per_action), e.g., (3, 3).
        na (int): Total action values per detection = num_actions * classes_per_action.
        cv4 (nn.ModuleList): Convolution layers for action prediction at each scale.
    """

    def __init__(self, nc: int = 80, action_shape: tuple = (3, 3), reg_max=16, end2end=False, ch: tuple = ()):
        """Initialize detection + action direction head.

        Args:
            nc (int): Number of object classes.
            action_shape (tuple): (num_actions, classes_per_action), e.g., (3, 3) for XYZ with {-1,0,1}.
            reg_max (int): Maximum number of DFL channels.
            end2end (bool): Whether to use end-to-end NMS-free detection.
            ch (tuple): Tuple of channel sizes from backbone feature maps.
        """
        super().__init__(nc, reg_max, end2end, ch)
        self.action_shape = action_shape  # (num_actions, classes_per_action)
        self.na = action_shape[0] * action_shape[1]  # total action outputs

        c4 = max(ch[0] // 4, self.na)
        self.cv4 = nn.ModuleList(
            nn.Sequential(Conv(x, c4, 3), Conv(c4, c4, 3), nn.Conv2d(c4, self.na, 1)) for x in ch
        )
        if end2end:
            self.one2one_cv4 = copy.deepcopy(self.cv4)
    ...
    def one2many(self):
        ...
        return dict(box_head=self.cv2, cls_head=self.cv3, action_head=self.cv4)

    @property
    def one2one(self):
        """Returns the one-to-one head components."""
        return dict(box_head=self.one2one_cv2, cls_head=self.one2one_cv3, action_head=self.one2one_cv4)

    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        """Decode bounding boxes, class probabilities, and action predictions."""
        preds = super()._inference(x)
        # Apply per-action-head softmax to action predictions
        actions = x["actions"]  # (batch, na, anchors)
        bs, _, nanchors = actions.shape
        num_actions, classes_per_action = self.action_shape
        actions = actions.view(bs, num_actions, classes_per_action, nanchors)
        actions = actions.softmax(dim=2)  # softmax per action head
        actions = actions.view(bs, self.na, nanchors)
        return torch.cat([preds, actions], dim=1)

    def forward_head(
        self, x: list[torch.Tensor], box_head, cls_head, action_head
    ) -> dict[str, torch.Tensor]:
        """Forward pass: predict boxes, classes, and action directions."""
        preds = super().forward_head(x, box_head, cls_head)
        if action_head is not None:
            bs = x[0].shape[0]
            preds["actions"] = torch.cat(
                [action_head[i](x[i]).view(bs, self.na, -1) for i in range(self.nl)], 2
            )
        return preds

    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        """Post-process predictions: boxes + scores + actions."""
        boxes, scores, actions = preds.split([4, self.nc, self.na], dim=-1)
        scores, conf, idx = self.get_topk_index(scores, self.max_det)
        boxes = boxes.gather(dim=1, index=idx.repeat(1, 1, 4))
        actions = actions.gather(dim=1, index=idx.repeat(1, 1, self.na))
        return torch.cat([boxes, scores, conf, actions], dim=-1)
...
class RTDETRDecoder(nn.Module):
    ...
    def forward(self, x: list[torch.Tensor], batch: dict | None = None) -> tuple | torch.Tensor:
        ...
        """
        from ImgAct.models.utils.ops import get_cdn_group

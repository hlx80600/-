...
from ImgAct.utils.checks import check_suffix
from ImgAct.utils.downloads import is_url
...
def check_class_names(names: list | dict) -> dict[int, str]:
    ...
    if isinstance(names, dict):
        ...
        if isinstance(names[0], str) and names[0].startswith("n0"):  # imagenet class codes, i.e. 'n01440764'
            from ImgAct.utils import ROOT, YAML
...
def default_class_names(data: str | Path | None = None) -> dict[int, str]:
    ...
    if data:
        try:
            ...
            from ImgAct.utils.checks import check_yaml
...
class AutoBackend(nn.Module):
    ...
    def warmup(self, imgsz: tuple[int, int, int, int] = (1, 3, 640, 640)) -> None:
        ...
        """
        from ImgAct.utils.nms import non_max_suppression

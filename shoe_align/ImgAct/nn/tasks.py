...
from ImgAct.nn.autobackend import check_class_names
from ImgAct.nn.modules import (
    AIFI,
    C1,
    C2,
    C2PSA,
...
from ImgAct.utils import DEFAULT_CFG_DICT, LOGGER, SETTINGS, WINDOWS, YAML, colorstr, emojis
from ImgAct.utils.checks import REMOTE_FILE_PREFIXES, check_file, check_requirements, check_suffix, check_yaml
from ImgAct.utils.loss import (
    E2ELoss,
    DiscreteMultiActionHeadLoss,
    MultiActionHeadLoss,
    PoseLoss26,
    ...
    v8PoseLoss,
    v8SegmentationLoss,
    TargetMultiActionLoss,
)
from ImgAct.utils.ops import make_divisible
from ImgAct.utils.patches import torch_load
from ImgAct.utils.plotting import feature_visualization
...
class DetectionModel(BaseModel):
    ...
    def init_criterion(self):
        ...
        if isinstance(self.model[-1], TargetMultiActionDetect):
            return (
                E2ELoss(self, TargetMultiActionLoss) if getattr(self, "end2end", False) else TargetMultiActionLoss(self)
            )
...
class ClassificationModel(BaseModel):
    ...
    def reshape_outputs(model, nc):
        ...
        if isinstance(m, (MultiActionHead, DiscreteMultiActionHead)):  # MultiActionHead head - skip reshape
            pass
        elif isinstance(m, Classify):  # YOLO Classify() head
    ...
    def init_criterion(self):
        ...
        # Check if the last module is a MultiActionHead head
        last_module = self.model[-1] if isinstance(self.model, torch.nn.Sequential) else list(self.model.children())[-1]
        
        if isinstance(last_module, DiscreteMultiActionHead):
            return DiscreteMultiActionHeadLoss(
                num_heads=last_module.num_heads,
                classes_per_head=last_module.classes_per_head,
            )
        elif isinstance(last_module, MultiActionHead):
            return MultiActionHeadLoss(
                num_heads=last_module.num_heads,
                classes_per_head=last_module.classes_per_head,
            )
...
class RTDETRDetectionModel(DetectionModel):
    ...
    def init_criterion(self):
        ...
        from ImgAct.models.utils.loss import RTDETRDetectionLoss
...
class WorldModel(DetectionModel):
    ...
    def get_text_pe(self, text, batch=80, cache_clip_model=True):
        ...
        """
        from ImgAct.nn.text_model import build_text_model
...
class YOLOEModel(DetectionModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from ImgAct.utils.loss import TVPDetectLoss
...
class YOLOESegModel(YOLOEModel, SegmentationModel):
    ...
    def loss(self, batch, preds=None):
        ...
        """
        if not hasattr(self, "criterion"):
            from ImgAct.utils.loss import TVPSegmentLoss
...
def torch_safe_load(weight, safe_only=False):
    ...
    """
    from ImgAct.utils.downloads import attempt_download_asset
...
def parse_model(d, ch, verbose=True):
    ...
    ch_override = d.get("ch")
    if ch_override is not None:
        ch = ch_override if not isinstance(ch_override, list) else ch_override[0]
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26, TargetMultiActionDetect}:
                m.legacy = legacy
    ...
    if isinstance(model, torch.nn.Module):  # PyTorch model
        ...
        for m in model.modules():
            if isinstance(m, SemanticSegment):
                ...
                return "segment"
            elif isinstance(m, (Classify, MultiActionHead, DiscreteMultiActionHead)):
                return "classify"

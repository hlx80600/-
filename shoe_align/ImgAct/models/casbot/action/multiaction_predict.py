...
class MultiActionHeadPredictor(BasePredictor):
    """Predictor for multi-action models."""
    ...
    def postprocess(self, preds, img, orig_imgs):
        """Process multi-action predictions to return Results objects."""
        ...
        if isinstance(last_layer, MultiActionHead):
        ...
        for pred, orig_img, img_path in zip(preds, orig_imgs, self.batch[0]):
            ...
            head_preds = reshaped.argmax(dim=1)
            head_confs = reshaped.max(dim=1).values

            result = Results(orig_img, path=img_path, names=self.model.names, probs=pred)
            result.multiaction_probs = reshaped

...
from ImgAct.utils import DEFAULT_CFG, LOGGER, RANK
...
class MultiActionHeadTrainer(BaseTrainer):
    """Trainer for multi-action models."""
    ...
    def __init__(self, cfg=DEFAULT_CFG, overrides=None, _callbacks=None):
        ...
        self.classes_per_head = overrides.pop("classes_per_head", 1)
        self.channels = overrides.pop("channels", 6)
        self.head_names = overrides.pop("head_names", None) or ["X", "Y", "Z"]
        self.class_names_per_head = overrides.pop("class_names", None) or [["val"]] * self.num_heads
        super().__init__(cfg, overrides, _callbacks)

    def set_model_attributes(self):
        """Set multi-action model attributes."""
        names = {}
        idx = 0
    ...
    def get_model(self, cfg=None, weights=None, verbose=True):
        """Return a ClassificationModel configured for multi-action output."""
        nc = self.num_heads * self.classes_per_head
        ch = self.data.get("channels", self.channels) if self.data else self.channels
        model = ClassificationModel(cfg, nc=nc, ch=ch, verbose=verbose and RANK == -1)
        if weights:
            model.load(weights)
    ...
    def setup_model(self):
        """Load or create model for multi-action classification."""
    ...
    def get_validator(self):
        ...
        return MultiActionHeadValidator(
            self.test_loader,
            self.save_dir,
            args=copy(self.args),
    ...
    def plot_training_samples(self, batch, ni):
        labels = {key: value for key, value in batch.items() if key != "cls"}
        labels["batch_idx"] = torch.arange(labels["img"].shape[0])
        plot_images(labels=labels, fname=self.save_dir / f"train_batch{ni}.jpg", on_plot=self.on_plot)

    def final_eval(self):
        """Bypass standard final evaluation to avoid AutoBackend 3-channel constraints."""
        model_path = self.best if self.best.exists() else None
        if self.args.save:
            from ImgAct.utils import RANK as dist_rank
            from ImgAct.utils.torch_utils import strip_optimizer, torch_distributed_zero_first as zero_first

            with zero_first(dist_rank):
                if dist_rank in {-1, 0}:
        ...
        if model_path:
            LOGGER.info(f"\nTraining completed! Best model saved to {self.best}.")

    def get_dataset(self):
        """Override to provide multi-action dataset info."""
        data = self.args.data

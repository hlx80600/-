...
from ImgAct.utils.checks import check_file, check_font, is_ascii
from ImgAct.utils.downloads import download, safe_download, unzip_file
from ImgAct.utils.ops import segments2boxes
...
def visualize_image_annotations(image_path: str, txt_path: str, label_map: dict[int, str]):
    ...
    from ImgAct.utils.plotting import colors
...
def convert_ndjson_to_yolo_if_needed(data: str | Path) -> str | Path:
    ...
    if data_str.endswith(".ndjson") or (data_str.startswith("ul://") and "/datasets/" in data_str):
        ...
        from ImgAct.data.converter import convert_ndjson_to_yolo
...
def check_cls_dataset(dataset: str | Path, split: str = "") -> dict[str, Any]:
    ...
    if not train_set.is_dir():
        ...
        if image_files := list(data_dir.rglob("*.jpg")) + list(data_dir.rglob("*.png")):
            from ImgAct.data.split import split_classify_dataset
...
class HUBDatasetStats:
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        for split in "train", "val", "test":
            ...
            if self.task == "classify":
                ...
                from ImgAct.data import YOLODataset
    ...
    def process_images(self) -> Path:
        ...
        from ImgAct.data import YOLODataset  # ClassificationDataset

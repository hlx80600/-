...
from ImgAct.data.utils import FORMATS_HELP_MSG, HELP_URL, IMG_FORMATS, check_file_speeds
from ImgAct.utils import DEFAULT_CFG, LOCAL_RANK, LOGGER, NUM_THREADS, TQDM
from ImgAct.utils.patches import imread
    ...
    def load_image(
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                im = imread(f, flags=self.cv2_flag)  # BGR
                if getattr(self, 'channels', 3) == 6:
                    h, w = im.shape[:2]
                    depth_path = str(f).replace('rgb', 'depth').replace('.jpg', '.png')
                    depth_im = cv2.imread(depth_path, cv2.IMREAD_GRAYSCALE)
                    if depth_im is None:
                        raise FileNotFoundError(f"Depth image not found at {depth_path}")
                    
                    # Normalize depth to [-1, 1], handling uint8 or uint16
                    z_max = 255.0 if depth_im.dtype == np.uint8 else 65535.0
                    z_grid = (depth_im.astype(np.float32) / (z_max / 2)) - 1.0
                    
                    # Create X and Y grids
                    x_linspace = np.linspace(-1, 1, w, dtype=np.float32)
                    y_linspace = np.linspace(-1, 1, h, dtype=np.float32)
                    xv, yv = np.meshgrid(x_linspace, y_linspace)
                    
                    # Convert original image to float32 and normalize roughly or just append raw if float is ok
                    im = im.astype(np.float32) # YOLO typically scales /255 later, but let's keep all 6 channels as float32
                    im = np.concatenate([im, xv[..., None], yv[..., None], z_grid[..., None]], axis=-1)

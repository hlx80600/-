...
from ImgAct.utils import LOCAL_RANK, LOGGER, NUM_THREADS, TQDM, colorstr
from ImgAct.utils.instance import Instances
from ImgAct.utils.ops import resample_segments, segments2boxes
from ImgAct.utils.torch_utils import TORCHVISION_0_18
...
class MultiActionHeadDataset:
    """Dataset for multi-head classification tasks (e.g., predicting X/Y/Z movement directions).
        ...
        root/
            train/
                images/
                    img001.jpg
                    ...
                labels.csv       # filename,head0,head1,head2  (class indices per head)
            val/
                images/
                    img001.jpg
                labels.csv

    The labels.csv should have columns: filename, then one column per head with integer class index.
    Example for X,Y,Z movement prediction (-1=0, 0=1, 1=2):
        filename,x,y,z
        img001.jpg,0,1,2
        img002.jpg,1,0,1

    Attributes:
        samples (list): List of (image_path, labels_tensor) tuples.
        torch_transforms (callable): PyTorch transforms for images.
        num_heads (int): Number of classification heads.
        classes_per_head (int): Number of classes per head.
        head_names (list[str]): Names for each head.
        class_names (list[list[str]]): Class names for each head.
    """

    def __init__(self, root: str, args, augment: bool = False, prefix: str = "",
                 num_heads: int = 3, classes_per_head: int = 3,
                 head_names: list[str] | None = None,
                 class_names: list[list[str]] | None = None):
        """Initialize multi-head classification dataset.

        Args:
            root (str): Root directory containing images/ folder and labels.csv.
            args: Configuration arguments with image size, augmentation params, etc.
            augment (bool): Whether to apply augmentations.
            prefix (str): Logging prefix.
            num_heads (int): Number of classification heads.
            classes_per_head (int): Number of classes per head.
            head_names (list[str], optional): Names for heads, e.g., ['X', 'Y', 'Z'].
            class_names (list[list[str]], optional): Class names per head, e.g., [['-1','0','1'], ...].
        """
        import csv
        ...
        self.class_names = class_names or [[str(j) for j in range(classes_per_head)] for _ in range(num_heads)]
        self.prefix = colorstr(f"{prefix}: ") if prefix else ""
        self.root = root
        ...
        if not labels_path.exists():
            raise FileNotFoundError(
                ...
                f"Multi-head classification requires a labels.csv in each split directory."
            )

        images_dir = Path(root) / "rgb"
        self.samples = []
        with open(labels_path, "r") as f:
            ...
            for row in reader:
                fname = row[0]
                labels = [float(x) for x in row[1: 1 + num_heads]]
                img_path = str(images_dir / fname)
                if Path(img_path).exists():
                    self.samples.append((img_path, labels))

        if not self.samples:
            raise FileNotFoundError(f"No valid samples found in {root}")

        if augment and hasattr(args, "fraction") and args.fraction < 1.0:
            self.samples = self.samples[: round(len(self.samples) * args.fraction)]

        LOGGER.info(f"{self.prefix}Loaded {len(self.samples)} samples with {num_heads} heads")
    ...
    def __getitem__(self, i: int) -> dict:
        """Return transformed 6-channel image (RGB + pseudo-color depth) and regression labels.

        Returns:
            (dict): {"img": tensor of shape (6, H, W), "cls": tensor of shape (num_heads,) with float values}.
        """
        ...
        # Load corresponding depth image and convert to pseudo-color
        depth_path = img_path.replace("/rgb/", "/depth/").replace("_rgb.png", "_depth.png")
        depth_im = cv2.imread(depth_path, cv2.IMREAD_GRAYSCALE)
        if depth_im is None:
            raise FileNotFoundError(f"Depth image not found: {depth_path}")
        pseudo_color = cv2.applyColorMap(depth_im, cv2.COLORMAP_JET)  # BGR 3-channel

        # Convert BGR to RGB for both
        im_rgb = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        pseudo_rgb = cv2.cvtColor(pseudo_color, cv2.COLOR_BGR2RGB)

        # Apply transforms to both images (same geometric transforms)
        im_pil = Image.fromarray(im_rgb)
        pseudo_pil = Image.fromarray(pseudo_rgb)
        im_tensor = self.torch_transforms(im_pil)        # (3, H, W)
        pseudo_tensor = self.torch_transforms(pseudo_pil)  # (3, H, W)

        # Concatenate to 6-channel tensor
        sample = torch.cat([im_tensor, pseudo_tensor], dim=0)  # (6, H, W)
        return {"img": sample, "cls": torch.tensor(labels, dtype=torch.float32)}
...
class DiscreteMultiActionHeadDataset:
    """Dataset for RGB discrete multi-action classification tasks.
    ...
    Expects directory structure:
        root/
            rgb/
                img001.jpg
                ...
            labels.csv       # filename,head0,head1,head2  (integer class indices per head)
    ...
    def __init__(
        ...
        prefix: str = "",
        num_heads: int = 3,
        classes_per_head: int = 3,
        head_names: list[str] | None = None,
        class_names: list[list[str]] | None = None,
    ):
        """Initialize discrete multi-action classification dataset."""
        import csv
        ...
        with open(labels_path, "r") as f:
            ...
            for row in reader:
                fname = row[0]
                labels = [int(float(x)) for x in row[1 : 1 + num_heads]]
                if len(labels) != num_heads:
                    raise ValueError(f"Expected {num_heads} labels per sample in {labels_path}, got {len(labels)}")
                if any(label < 0 or label >= classes_per_head for label in labels):
                    raise ValueError(
                        f"Label values must be in [0, {classes_per_head - 1}] for {labels_path}, got {labels}"
                    )
                img_path = str(images_dir / fname)
                if Path(img_path).exists():
                    self.samples.append((img_path, labels))
    ...
    def __getitem__(self, i: int) -> dict:
        """Return transformed RGB image and discrete labels."""
        img_path, labels = self.samples[i]
        im = cv2.imread(img_path)
        if im is None:
            raise FileNotFoundError(f"Image not found: {img_path}")
        ...
        im_rgb = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
        im_pil = Image.fromarray(im_rgb)
        sample = self.torch_transforms(im_pil)  # (3, H, W)
        return {"img": sample, "cls": torch.tensor(labels, dtype=torch.long)}
...
class TargetMultiActionDataset(YOLODataset):
    """Dataset for TargetMultiActionDetect: YOLO detection + per-object action labels.

    Label format (per line in txt file):
        class x_center y_center width height action_0 action_1 ... action_n

    Where action_i are integer class indices (e.g., 0, 1, 2 for -1, 0, 1).
    The number of action columns must match num_actions from the data config.

    Data config (data.yaml) must include:
        action_shape: [3, 3]  # [num_actions, classes_per_action]
        action_names: ["X", "Y", "Z"]  # optional, names per action head
    """

    def __init__(self, *args, data=None, task="detect", **kwargs):
        """Initialize with action shape from data config."""
        self.action_shape = data.get("action_shape", [3, 3])
        self.num_actions = self.action_shape[0]
        super().__init__(*args, data=data, task=task, **kwargs)

    def cache_labels(self, path):
        """Cache labels with action columns parsed from label files."""
        x = {"labels": []}
        nm, nf, ne, nc, msgs = 0, 0, 0, 0, []
        desc = f"{self.prefix}Scanning {path.parent / path.stem}..."
        total = len(self.im_files)
        num_cls = len(self.data["names"])
        expected_cols = 5 + self.num_actions  # class + xywh + actions

        from ImgAct.data.utils import check_image

        for im_file, lb_file in zip(self.im_files, self.label_files):
            try:
                msg, shape = check_image(im_file)
                msg = f"{self.prefix}{msg}" if msg else ""

                actions = None
                if Path(lb_file).is_file():
                    nf += 1
                    with open(lb_file, encoding="utf-8") as f:
                        lb = [line.split() for line in f.read().strip().splitlines() if len(line.strip())]
                    lb = np.array(lb, dtype=np.float32)

                    if nl := len(lb):
                        if lb.shape[1] < 5:
                            raise ValueError(f"labels need at least 5 columns, got {lb.shape[1]}")
                        if lb.shape[1] >= expected_cols:
                            actions = lb[:, 5:5 + self.num_actions].astype(np.int64)
                        elif lb.shape[1] > 5:
                            LOGGER.warning(
                                f"{self.prefix}{im_file}: expected {expected_cols} cols, got {lb.shape[1]}, "
                                f"padding actions with zeros"
                            )
                            actions = np.zeros((nl, self.num_actions), dtype=np.int64)
                        else:
                            actions = np.zeros((nl, self.num_actions), dtype=np.int64)

                        # Validate bbox coords
                        points = lb[:, 1:5]
                        assert points.max() <= 1.01, f"non-normalized coordinates {points[points > 1.01]}"
                        assert lb[:, :5].min() >= -0.01, f"negative values {lb[lb < -0.01]}"

                        max_cls = lb[:, 0].max()
                        assert max_cls < num_cls, f"class {int(max_cls)} >= nc={num_cls}"

                        _, i = np.unique(lb[:, :5], axis=0, return_index=True)
                        if len(i) < nl:
                            lb = lb[i]
                            actions = actions[i] if actions is not None else None
                            msg = f"{self.prefix}{im_file}: {nl - len(i)} duplicate labels removed"
                    else:
                        ne += 1
                        lb = np.zeros((0, 5), dtype=np.float32)
                        actions = np.zeros((0, self.num_actions), dtype=np.int64)
                else:
                    nm += 1
                    lb = np.zeros((0, 5), dtype=np.float32)
                    actions = np.zeros((0, self.num_actions), dtype=np.int64)
    ...
    def update_labels_info(self, label):
        """Store actions separately before creating Instances."""
        actions = label.pop("actions", None)
        label = super().update_labels_info(label)
        label["actions"] = actions
        return label

    def build_transforms(self, hyp=None):
        """Build transforms, adding action passthrough."""
        transforms = super().build_transforms(hyp)
        # Add action formatting at the end
        transforms.append(_ActionFormat(self.num_actions))
        return transforms

    @staticmethod
    def collate_fn(batch):
        """Collate with action tensors."""
        new_batch = YOLODataset.collate_fn(batch)
        # Concatenate actions across batch items
        if "actions" in batch[0]:
            new_batch["actions"] = torch.cat([b["actions"] for b in batch], 0)
        return new_batch


class _ActionFormat:
    """Transform that converts action labels to tensor format."""

    def __init__(self, num_actions):
        self.num_actions = num_actions

    def __call__(self, labels):
        actions = labels.pop("actions", None)
        if actions is not None and len(actions) > 0:
            labels["actions"] = torch.from_numpy(np.asarray(actions)).long()
        else:
            labels["actions"] = torch.zeros(0, self.num_actions, dtype=torch.long)
        return labels

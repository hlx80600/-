...
from ImgAct.utils.metrics import CITYSCAPES_WEIGHT, OKS_SIGMA, RLE_WEIGHT
from ImgAct.utils.ops import crop_mask, xywh2xyxy, xyxy2xywh
from ImgAct.utils.tal import RotatedTaskAlignedAssigner, TaskAlignedAssigner, dist2bbox, dist2rbox, make_anchors
from ImgAct.utils.torch_utils import autocast
...
class TargetMultiActionLoss(v8DetectionLoss):
    """Loss for TargetMultiActionDetect: detection + per-object action direction prediction.

    Extends v8DetectionLoss with per-action-head cross-entropy loss for foreground anchors.
    Loss components: [box, cls, dfl, action].
    """

    def __init__(self, model, tal_topk=10, tal_topk2=None):
        """Initialize with action shape from the detection head."""
        super().__init__(model, tal_topk, tal_topk2)
        m = model.model[-1]
        self.action_shape = m.action_shape
        self.na = m.na

    def loss(self, preds, batch):
        """Calculate detection + action loss."""
        pred_actions = preds["actions"].permute(0, 2, 1).contiguous()
        ...
        if fg_mask.sum():
            ...
            target_actions = self._select_target_actions(actions, batch_idx, target_gt_idx, fg_mask)

            fg_pred = pred_actions[fg_mask].view(-1, num_actions, classes_per_action)
            fg_target = target_actions[fg_mask]

            action_loss = sum(
                F.cross_entropy(fg_pred[:, i, :], fg_target[:, i], reduction="mean")
                for i in range(num_actions)
            ) / num_actions
            loss[3] = action_loss

        loss[3] *= getattr(self.hyp, "action", 1.0)
        return loss * batch_size, loss.detach()

    def _select_target_actions(self, actions, batch_idx, target_gt_idx, masks):
        """Select target actions for each anchor based on assignment."""
        batch_idx = batch_idx.flatten().long()
        batch_size = len(masks)
        num_actions = actions.shape[1]

        max_gt = torch.unique(batch_idx, return_counts=True)[1].max()
        batched = torch.zeros(batch_size, max_gt, num_actions, device=actions.device, dtype=actions.dtype)

        offsets = torch.zeros(batch_size + 1, dtype=torch.long, device=actions.device)
        offsets.scatter_add_(0, batch_idx + 1, torch.ones_like(batch_idx))
        offsets = offsets.cumsum(0)
        within_idx = torch.arange(len(batch_idx), device=actions.device) - offsets[batch_idx]
        batched[batch_idx, within_idx] = actions

        idx = target_gt_idx.unsqueeze(-1).expand(-1, -1, num_actions)
        return batched.gather(1, idx)

class MultiActionHeadLoss:
    """Criterion class for computing multi-head regression losses.

    Computes independent SmoothL1 loss for each regression head and averages them.
    Expects batch["cls"] to be a tensor of shape (batch, num_heads) with float regression targets.
    """

    def __init__(self, num_heads: int = 3, classes_per_head: int = 1):
        """Initialize with number of heads and output size per head."""
        self.num_heads = num_heads
        self.classes_per_head = classes_per_head

    def __call__(self, preds: Any, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        """Compute multi-head regression loss (SmoothL1)."""
        preds = preds[1] if isinstance(preds, (list, tuple)) else preds
        # preds shape: (batch, num_heads * classes_per_head)
        # batch["cls"] shape: (batch, num_heads) with float regression targets
        b = preds.shape[0]
        preds = preds.view(b, self.num_heads, self.classes_per_head).squeeze(-1)  # (batch, num_heads)
        targets = batch["cls"].float()  # (batch, num_heads)
        total_loss = F.smooth_l1_loss(preds, targets, reduction="mean")
        return total_loss, total_loss.detach()


class DiscreteMultiActionHeadLoss:
    """Criterion class for computing multi-head classification losses.

    Computes independent CrossEntropy loss for each classification head and averages them.
    Expects batch["cls"] to be a tensor of shape (batch, num_heads) with long category targets.
    """

    def __init__(self, num_heads: int = 3, classes_per_head: int = 3):
        """Initialize with number of heads and output size per head."""
        self.num_heads = num_heads
        self.classes_per_head = classes_per_head

    def __call__(self, preds: Any, batch: dict[str, torch.Tensor]) -> tuple[torch.Tensor, torch.Tensor]:
        """Compute multi-head classification loss (CrossEntropy)."""
        preds = preds[1] if isinstance(preds, (list, tuple)) else preds
        # preds shape: (batch, num_heads * classes_per_head)
        # batch["cls"] shape: (batch, num_heads) with long class targets
        b = preds.shape[0]
        preds = preds.view(b, self.num_heads, self.classes_per_head)  # (batch, num_heads, classes)
        targets = batch["cls"].long()  # (batch, num_heads)
        
        total_loss = 0.0
        for i in range(self.num_heads):
            head_preds = preds[:, i, :]
            head_targets = targets[:, i]
            total_loss += F.cross_entropy(head_preds, head_targets, reduction="mean")

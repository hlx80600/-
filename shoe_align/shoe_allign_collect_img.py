"""鞋头对位采图：预览相机画面，按空格保存当前帧，按 q 退出。

Usage:
    python shoe_align/shoe_allign_collect_img.py --slot-config press_shoes/config/left_slot.yaml
"""
...
CMDT = 0.01  # ServoCart command period in seconds (e.g. 80ms for ~12.5Hz)
...
DEFAULT_TOE_ALIGN_MODEL = PROJECT_ROOT / "models/toe_align/0626ybest.pt"
DISPLAY_WINDOW = "toe_align"
SNAPSHOT_INTERVAL_S = 10
DEFAULT_SNAPSHOT_DIR = Path("/home/casbot/robot/Casbot_Press_Shoes/shoe_align_test_img/right02")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"存图目录 (default: {DEFAULT_SNAPSHOT_DIR})",
    )
...
class ShoeAlignController:
    ...
    def __init__(self, arm, camera,
            ...
            cmd_t: float = CMDT, step_size: float = STEP_SIZE, log: Any | None = None,):
        self.arm = arm
        self.camera = camera
    ...
    def save_snapshot_now(self, save_dir: str | Path | None = None) -> Path | None:
        """保存当前预览帧（由主线程在按空格时调用）。"""
        ...
        dir_path = Path(save_dir) if save_dir is not None else (self._save_dir or DEFAULT_SNAPSHOT_DIR)
        dir_path.mkdir(parents=True, exist_ok=True)
        dt = datetime.now()
        path = dir_path / (
            f"align_{dt.strftime('%m%d%H%M%S')}_{dt.strftime('%f')[:3]}"
        ...
        if cv2.imwrite(str(path), frame):
            self._log(f"已存图: {path}")
            return path
        self._log(f"存图失败: {path}")
        return None

    def predict_frame(self, frame) -> tuple[str, float]:
        tensor = fast_classify_preprocess(frame, self.imgsz).to(
            self._infer_device, non_blocking=self._non_blocking
        )
        with torch.inference_mode():
            logits = self._net(tensor)[0].view(self._num_heads, self._classes_per_head)
            head_logits = logits[0]
            top1_index = int(head_logits.argmax().item())
            confidence = float(torch.softmax(head_logits, dim=0).max().item())
        return str(top1_index), confidence
        ...
        try:
            while self.running:
                ...
                if color_image is None:
                    time.sleep(0.01)
                    continue
                label, confidence = self.predict_frame(color_image)
                if confidence > 0.92:
...
def main() -> None:
    ...
    controller.start_align_async(save_images=False, save_dir=output_dir)
    print(f"预览窗口: {DISPLAY_WINDOW}")
    print(f"按空格存图 -> {output_dir}")
    print("按 q 退出")
    try:
        while controller.running:
            ...
            if frame is not None:
                ...
                cv2.putText(
                    display,
                    f"label={label}  [space=save q=quit]",
                    (10, 30),
            ...
            if key == ord(" "):
                controller.save_snapshot_now(output_dir)
            elif key == ord("q"):

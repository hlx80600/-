...
CMDT = 0.1  # ServoCart command period in seconds (e.g. 80ms for ~12.5Hz)
...
class ShoeAlignController:
    ...
    def __init__(self, arm, camera,
            model_path: str | Path, dir_x: float, dir_y: float, imgsz: int = 256, device: str | None = None,
    ...
    def _update_lateral_label(self, label: str, confidence: float, last_label: str) -> None:
        """更新左右方向 label，不参与伺服运动。"""
        if label == "-1":
            return
        if confidence > 0.92:
            self.set_lateral_label(label)
        if label in ("1", "2") and confidence > 0.95:
            self.set_lateral_label(label)
        if label == "0":
            self.set_lateral_label(label)
    ...
    def _robot_control_loop(self):
        ...
        while self.running:
            ...
            if label == "1":
                inc_pose = [self.step_size * self.dir_x * self.speed_scale_factor, self.step_size * self.dir_y * self.speed_scale_factor, 0.0, 0.0, 0.0, 0.0]
...
def main() -> None:
    ...
    print(f"左/右方向(仅识别不移动): dx={left_dir_x:.4f}, dy={left_dir_y:.4f}")
    ...
    try:
        while controller.running:
            ...
            if frame is not None:
                ...
                lat_text = {"0": "中", "1": "左", "2": "右"}.get(lat_label, lat_label)
                cv2.putText(
                    display,

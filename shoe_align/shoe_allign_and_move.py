...
CMDT = 2  # ServoCart command period in seconds (e.g. 80ms for ~12.5Hz)
...
class ShoeAlignController:
    ...
    def _setup_inference(self) -> None:
        """预热模型并缓存推理所需句柄，避免每帧走完整 predict 流水线。"""
    ...
    def predict_frame(self, frame) -> tuple[str, float]:
        ...
        tensor = fast_classify_preprocess(infer_frame, self.imgsz).to(self.model.device)
        with torch.inference_mode():
            logits = self._net(tensor)[0].view(self._num_heads, self._classes_per_head)
            head_probs = logits.softmax(dim=1)
            top1_index = int(head_probs[0].argmax().item())
            confidence = float(head_probs[0].max().item())
    ...
    def _robot_control_loop(self):
        ...
        while self.running and self._servo_active:
            ...
            if label == "1":
                inc_pose = [
                    self.step_size * self.dir_x * self.speed_scale_factor,
                    self.step_size * self.dir_y * self.speed_scale_factor,
                    0.0, 0.0, 0.0, 0.0,
                ]
                ...
                # ret = self.arm.RobotServoSpline(points=segmented_poses,speed_mm_s=300,max_ori_step_deg=0.2,tool=MOVE_CART_TOOL,user=MOVE_CART_USER)
                # print(f"鞋槽对位时间: {align_time}秒")
    ...
    def _run_vision_loop(self):
        ...
        try:
            ...
            while self.running:
                ...
                self._log(f"前后: {label}, confidence: {confidence:.2f},推理时间: {time.perf_counter() - time_start:.3f}s")
                if confidence > 0.92:
...
def main() -> None:
    ...
    print(f"左旋转90°方向: dx={left_dir_x:.4f}, dy={left_dir_y:.4f}")
        ...
        while controller.running:
            ...
            if frame is not None:
                ...
                if now - last_snapshot >= SNAPSHOT_INTERVAL_S:
                    path = snapshot_dir / f"{int(time.time() * 1000)}_label{label}.jpg"

...
bash tools/install_autostart.sh disable  # 关闭
```

也可在 HMI **设置 → 界面与刷新** 勾选「开机登录后自动启动本程序」并保存。这只在**当前用户登录进桌面之后**拉起程序；工控机请同时打开系统自动登录，否则会停在登录界面。登录后约 8 秒启动，避免与桌面抢 DISPLAY。

1. 读 [docs/从零看懂本程序.md](docs/从零看懂本程序.md)  
2. Mock 空跑：运行监控 → 初始化 → 启动 →「模拟光电感应到位」  
3. 接真机前：左侧 **本机设备** 对照网卡 IP / USB serial → [docs/操作说明.md](docs/操作说明.md) §1 改 IP → HMI **设置 → 通信与设备** 保存 → **重启程序**  

"""视觉调试：实时预览 / 内参 / YOLO 测试；四台相机可单独开/关 Mock。
...
class VisionPage(QWidget):
    ...
    def __init__(self, coord: Coordinator):
        ...
        # currentTextChanged 延后连接，避免构造未完成时回调
        ...
        for b in (b7, b8, b9, b_solve):
            biz_row2.addWidget(b, 0)
        biz_row2.addStretch(1)
        biz_lay.addLayout(biz_row2)
        biz_row3 = QHBoxLayout()
        for b in (b_kjson, b_roij, b_pick, b_ml):
            biz_row3.addWidget(b, 0)
        biz_row3.addStretch(1)
        biz_lay.addLayout(biz_row3)
        he_clr = QHBoxLayout()
        he_clr.addWidget(b_del_he_s, 0)
        he_clr.addWidget(b_del_he, 0)
        he_clr.addStretch(1)
        biz_lay.addLayout(he_clr)
        self.lbl_handeye = QLabel("-")
        biz_lay.addWidget(self.lbl_handeye)
        root.addWidget(biz)
        ...
        self._log(
            "提示：完整采图/训练流程见「视觉采图」。本页负责出图、棋盘格、点像素采手眼。"
        )
    ...
    def _append_log(self, text: str) -> None:
        cur = self.txt_result.toPlainText().strip()
        self.txt_result.setPlainText((cur + "\n" + text) if cur else text)
    ...
    def _refresh_commission(self) -> None:
        ...
        if hasattr(self, "lbl_stack"):
            self.lbl_stack.setText(vcomm.stack_line(self.ctx))
        if hasattr(self, "lbl_models"):
            self.lbl_models.setText(vcomm.models_list_text(self.ctx))
    ...
    def _show_bgr(self, vis) -> None:
        ...
        self.preview.set_display_geom(w_, h_, dx, dy, dw, dh)
    ...
    def refresh(self) -> None:
        ...
        if holding:
            vis = self._board_hold_vis.copy()
            img = vis
        elif self._board_busy:
            img = self._grab()
            if img is None:
                vis = self._last_bgr.copy() if self._last_bgr is not None else None
                if vis is None:
                    return
                img = vis
            else:
            ...
            if self._overlay_toe is not None and self._cam_id() == "cam1":
                gx, gy, tx, ty, ln = self._overlay_toe
                p0 = (int(gx), int(gy))
                p1 = (int(tx), int(ty))
                cv2.circle(vis, p0, 6, (0, 255, 255), 2)
                cv2.circle(vis, p1, 6, (0, 0, 255), 2)
                cv2.arrowedLine(vis, p0, p1, (0, 0, 255), 2, tipLength=0.12)
                cv2.putText(
                    vis,
                    f"L={ln:.0f}mm",
            ...
            if self._pending_pixel is not None:
                ...
                cv2.drawMarker(vis, (pu, pv), (255, 0, 255), cv2.MARKER_CROSS, 16, 2)
        x, y, w, h = self.sp_x.value(), self.sp_y.value(), self.sp_w.value(), self.sp_h.value()

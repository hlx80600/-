...
class PayloadPage(QWidget):
    ...
    def _robot_box(self, robot_key: str, title: str) -> QGroupBox:
        ...
        lay = QVBoxLayout(box)
        for mode, caption in (
            ("empty", "负载1 / 工具1 · 手爪（未抓鞋）"),
            ("with_shoe", "负载2 / 工具2 · 手爪+鞋（抓鞋）"),
        ):
            g = QGroupBox(caption)
            form = QFormLayout(g)
            mass = _spin_mass()
        ...
        btn_empty = QPushButton("切到工具1")
        style_button(btn_empty, "neutral")
        btn_empty.clicked.connect(lambda: self._force_mode(robot_key, False))
        btn_shoe = QPushButton("切到工具2")
        style_button(btn_shoe, "warn")
    ...
    def _refresh_status(self) -> None:
        for rk, robot in (("robot1", self.ctx.robot1), ("robot2", self.ctx.robot2)):
            ...
            tcp = p.get("tcp", [])
            lbl.setText(
                f"当前: {mode} | tool={robot.tool} | "
                f"load={p.get('load_num', '-')} | "
